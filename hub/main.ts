...
const games = [
  ['m1-1-mission-loadout','M1.1','Mission Loadout: Build and Clear'],
  ['m1-2-site-mission-control','M1.2','Site Mission Control: Plan, Fly, Adapt'],
  ['m2-2-thermal-triage','M2.2','Thermal Triage: Case File'],
  ['m3-2-digital-handover','M3.2','Digital Handover Room: Integration Gate']
] as const;
const app=document.getElementById('root')!;
const locale=new URLSearchParams(location.search).get('lang')||'en';
app.innerHTML=`<main class="shell"><div class="eyebrow">Development and partner QA</div><h1>Drone4Build Test Hub</h1><p class="muted">Launch a game directly, use the local SCORM mock, or inspect stored mock values. Packages are generated by <code>npm run package:scorm</code>.</p><label for="lang">Locale</label> <input id="lang" value="${locale}" maxlength="12"/><div id="games" class="hub-grid"></div><section class="panel"><h2>Mock SCORM values</h2><pre id="inspector"></pre><button id="refresh">Refresh inspector</button> <button id="reset">Reset all stored progress</button></section></main>`;
